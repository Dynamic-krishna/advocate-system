...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_details'])) {
        try {
            ...
            if ($doe && !strtotime($doe)) {
                throw new Exception("Invalid date of enrollment");
            }
            ...
            if (isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
                ...
                if (!in_array($ext, $allowed)) {
                    ...
                    throw new Exception("File size must be between 20KB and 500KB");
                } else {
                    $upload_dir = 'uploads/';
            ...
            if ($details) {
                $sql = "UPDATE advocate_details SET date_of_birth = ?, date_of_enrollment = ?, updated_at = NOW()";
                $params = [$dob, $doe];
                if ($photo_path) {
                    // Delete old photo if exists
                    if ($details['photograph_path'] && file_exists($details['photograph_path'])) {
                        unlink($details['photograph_path']);
                    }
                    $sql .= ", photograph_path = ?";
                    $params[] = $photo_path;
                }
                ...
                $stmt->execute([$_SESSION['user_id'], $dob, $doe, $photo_path]);
            }
            
            $pdo->commit();
            
            // Redirect with success message
            $_SESSION['flash_message'] = "Details saved successfully!";
...
<body>
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <input type="date" name="dob" value="<?php echo $details ? htmlspecialchars($details['date_of_birth']) : ''; ?>">
        </div>

        <div class="form-group">
            <label>Date of Enrollment:</label>
            <input type="date" name="doe" value="<?php echo $details ? htmlspecialchars($details['date_of_enrollment']) : ''; ?>">
        </div>
        ...
        <div class="form-group">
            ...
            <?php if ($details && $details['photograph_path']): ?>
                <div class="current-photo">
                    Current photo: <img src="<?php echo htmlspecialchars($details['photograph_path']); ?>" height="50">
                </div>
            <?php endif; ?>
        </div>
